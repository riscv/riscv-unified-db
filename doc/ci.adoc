...
These tests are specified in the same xref:.github/workflows/regress.yml[`.github/workflows/regress.yml`] as the PR tests, distinguished by `ci_stage: merge_queue` in xref:tools/test/regress-tests.yaml[`tools/test/regress-tests.yaml`], and are uploaded via xref:.github/workflows/pages.yml[`.github/workflows/pages.yml`].
