...
The following example shows how the Branch if Less Than Unsigned (`BLTU`) instruction is specified in IDL. `xs1`, `xs2`, and `imm` are decode fields extracted automatically from the instruction encoding before execution.
...
3. Check if unsigned `src1` is less than unsigned `src2`.
