...
module UdbGen
  module ExtDocHelpers
    ...
    def copyright_year(version)
      version.ratification_date.nil? ? Date.today.year.to_s : version.ratification_date.split("-")[0]
    end
    ...
    def company_name(ext)
      ext.company.nil? ? "unknown" : ext.company.name
    end

    # Returns the doc license name or "unknown".
    sig { params(ext: Udb::Extension).returns(String) }
    def doc_license_name(ext)
      ext.doc_license.nil? ? "unknown" : ext.doc_license["name"]
    end

    # Returns the doc license URL or "unknown".
    sig { params(ext: Udb::Extension).returns(String) }
    def doc_license_url(ext)
      ext.doc_license.nil? ? "unknown" : ext.doc_license["url"]
    end
    ...
    def riscv_branded?(ext)
      !ext.company.nil? && !(ext.company.name =~ /RISCV/).nil?
    end
    ...
    def sorted_contributors(version)
      version.contributors.sort { |a, b| a.name.split(" ").last <=> b.name.split(" ").last }
    end
