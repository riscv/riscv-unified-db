...
# This extconf.rb runs at `gem install` time to download the espresso binary
...
# Load ESPRESSO_VERSION from the sibling lib file without requiring the full gem
espresso_version_rb = File.expand_path("../../lib/udb/espresso_version.rb", __dir__)
load espresso_version_rb
...
dest_dir  = File.join(xdg_cache, "udb", "espresso", ESPRESSO_VERSION, cpu)
dest_file = File.join(dest_dir, "espresso")

unless File.exist?(dest_file)
  FileUtils.mkdir_p(dest_dir)

  asset_name = "espresso-#{cpu}"
  url_str = "https://github.com/#{GITHUB_REPO}/releases/download/#{ESPRESSO_VERSION}/#{asset_name}"
