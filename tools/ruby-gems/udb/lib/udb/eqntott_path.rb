...
require "net/http"
require "uri"
require_relative "eqntott_version"
...
module Udb
  ...
  module EqntottPath
    ...
    class << self
      # Returns the absolute path to the eqntott binary.
      # Attempts to download the binary if not present.
      # Raises only if the download or installation fails.
      def binary
        path = File.join(bin_dir, "eqntott")
        unless File.exist?(path)
          begin
            download_binary(path)
          rescue => e
            raise "eqntott binary not found at #{path} and download failed: #{e.message}"
          end
        end
        path
      end
      ...
      def bin_dir
        ...
        File.join(xdg_cache, "udb", "eqntott", Udb::EQNTOTT_VERSION, cpu)
      end
      ...
      def download_binary(dest_file)
        ...
        asset_name = "eqntott-#{cpu}"
        url_str = "https://github.com/#{GITHUB_REPO}/releases/download/#{Udb::EQNTOTT_VERSION}/#{asset_name}"

        $stderr.puts "Downloading eqntott (#{Udb::EQNTOTT_VERSION}, #{cpu}) from GitHub releases..."
      ...
      def download_with_redirects(url_str, limit = 10)
        ...
        request["User-Agent"] = "udb-gem/eqntott-installer"
