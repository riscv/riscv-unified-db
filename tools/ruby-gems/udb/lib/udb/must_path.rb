  ...
  module MustPath
    ...
    class << self
      ...
      def binary
        ...
        unless File.exist?(path)
          begin
            ...
            raise "must binary not found at #{path} and download failed: #{e.message}"
          end
        end
        path
      end
      ...
      def download_binary(dest_file)
        ...
        asset_name = "must-#{cpu}"
        url_str = "https://github.com/#{GITHUB_REPO}/releases/download/#{Udb::MUST_VERSION}/#{asset_name}"

        $stderr.puts "Downloading must (#{Udb::MUST_VERSION}, #{cpu}) from GitHub releases..."
