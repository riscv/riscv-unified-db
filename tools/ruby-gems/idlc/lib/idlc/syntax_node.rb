...
module Treetop
  module Runtime
    class SyntaxNode
      ...
      sig { params(filename: T.nilable(String), starting_line: Integer, starting_offset: Integer).void }
      def set_input_file(filename, starting_line = 0, starting_offset = 0)
        ...
        elements&.each do |child|
          ...
          child.set_input_file(filename, starting_line, child_offset)
        end
      ...
      def set_input_file_unless_already_set(filename, starting_line = 0, starting_offset = 0)
        if @input_file.nil?
          set_input_file(filename, starting_line, starting_offset)
        end
      end
    end
  end
