...
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../backends/generators"))
)
...
def extract_csr_addr(cond):
    ...
    for p in parts:
        ...
        if left == "csr":
            try:
                csr_addr = int(right, 0)  # parse both dec and hex addresses
            except ValueError:
                return None
            continue
...
def main():
    pkg_version = version("capstone")
    logging.info("Using capstone version %s", pkg_version)

    parser = argparse.ArgumentParser(
        description="Compare generated CSR switch case to Capstone disassembly output."
    )
    parser.add_argument(
        "--csr_switch",
    ...
    def is_unhandled_case(t):
        if t[0] in unhandled_cases_dict:
            logging.info(f"Ignoring case - {t[0]}. Reason: {unhandled_cases_dict[t[0]]}")
            return True
        else:
            return False
    ...
    if diff:
        logging.info("CSRs missing in switch statement: %s", diff)
        sys.exit(1)

    logging.info("Regression test passed for Capstone generator")
