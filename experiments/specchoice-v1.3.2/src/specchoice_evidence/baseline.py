...
def validate_boundary_restart(baseline_path: Path, previous_baseline_path: Path, allowlist_path: Path, incident_receipt_path: Path) -> dict[str, object]:
    ...
    if restart.get("previous_baseline", {}).get("sha256") != previous_sha or restart.get("incident_receipt", {}).get("sha256") != sha256_bytes(raw_incident) or restart.get("v5_allowlist", {}).get("sha256") != sha256_bytes(raw_allow): raise BaselineError("BOUNDARY_RESTART_BINDING_MISMATCH")
