...
Both also apply to element schemas nested under ``items``, ``additionalItems``
and ``contains``, not just to the parameter's own ``schema``. Array-valued
parameters put the interesting constraints there, so a check that stopped at the
top level would miss exactly the places a typo is most likely to hide.
"""
...
# Keywords that give an integer a bounded value set. An integer carrying none
# of these accepts every value the host can represent.
...
# Keywords whose value is itself a schema, or a list of schemas. A parameter
# describing an array puts its element schema under one of these, so the strict
# checks have to follow them to reach the whole document.
_SUBSCHEMA_KEYWORDS = ("items", "additionalItems", "contains")


def _check_subschema(subschema: Any, path: str) -> list[str]:
    """Apply the strict checks to ``subschema`` and everything nested in it.

    ``path`` names the location being checked so a problem in an element schema
    is not reported as though it were on the parameter itself.
    """
    ...
    if unknown:
        problems.append(
            f"{path}: unknown draft-07 keyword(s) {unknown}. "
            "param_schema cannot currently reject these (see issue #2173), "
            "but they are silently ignored by every validator."
        )

    if subschema.get("type") == "integer" and not (BOUNDING_KEYWORDS & set(subschema)):
        problems.append(
            f"{path}: integer with no enum, const, bounds or $ref is unconstrained; "
            "give it a range or an enum (compare issues #2170 and #2102)."
        )

    for keyword in _SUBSCHEMA_KEYWORDS:
        nested = subschema.get(keyword)
        if isinstance(nested, dict):
            problems += _check_subschema(nested, f"{path}/{keyword}")
        elif isinstance(nested, list):
            for index, entry in enumerate(nested):
                problems += _check_subschema(entry, f"{path}/{keyword}/{index}")
...
def validate_doc(doc: dict[str, Any], *, strict: bool = True) -> list[str]:
    ...
    schema does not currently enforce. They apply to the parameter's own schema
    and to any element schema nested under it.
    """
    problems: list[str] = []
    ...
    return problems + _check_subschema(doc.get("schema"), "schema")
