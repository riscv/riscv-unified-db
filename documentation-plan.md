...
Explain the Rake task structure, how to get a `ConfiguredArchitecture` object, how ERB templates work, and how to use `udb_helpers`.
...
| `proc_cert_class` | Processor certification classes |
| `proc_cert_model` | Processor certification models |
| `profile` | RISC-V profiles |
| `profile_family` | Profile families |
| `profile_release` | Profile releases |
| `prose` | Prose documentation fragments |
...
| `udb-gen` gem | `tools/ruby-gems/udb-gen/` | Artifact generation orchestrator |
| `idlc` gem | `tools/ruby-gems/idlc/` | IDL compiler; CLI and AST API |
| `udb_helpers` gem | `tools/ruby-gems/udb_helpers/` | Template helpers used by backends |
| `idl_highlighter` gem | `tools/ruby-gems/idl_highlighter/` | Syntax highlighting for IDL |
| `bin/generate` | `bin/generate` | Wrapper to run content generators |
...
Explain all the generators that exist in the monorepo. Show examples of what they produce and how to run them.

| Backend | Location | What it produces |
|---|---|---|
| `cfg_html_doc` | `backends/cfg_html_doc/` | HTML documentation for a specific configuration |
| `cpp_hart_gen` | `backends/cpp_hart_gen/` | C++ ISS (Instruction Set Simulator) hart implementation |
| `generators/c_header` | `backends/generators/c_header/` | C encoding header (used by Spike, ACTs, Sail) |
| `generators/go` | `backends/generators/Go/` | Go instruction/CSR definitions |
| `generators/sverilog` | `backends/generators/sverilog/` | SystemVerilog decode package |
| `indexer` | `backends/indexer/` | Search index for the UDB documentation website |
| `instructions_appendix` | `backends/instructions_appendix/` | AsciiDoc instruction appendix |
| `prm_pdf` | `backends/prm_pdf/` | Programmer's Reference Manual PDF |
| `proc_crd` | `backends/proc_crd/` | Processor Certification Requirements Document |
| `proc_ctp` | `backends/proc_ctp/` | Processor Certification Test Plan |
| `profile` | `backends/profile/` | Profile documentation |

Note: `backends/portfolio/` and `backends/proc_cert/` are shared code libraries used by multiple backends, not standalone generators.
