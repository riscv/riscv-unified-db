...
A C compiler (GCC or Clang) is required. The `idlc` gem includes a native
extension—the tree-sitter IDL grammar—that `bin/setup` compiles from source,
and that is also compiled when the published `idlc`/`udb` gems are installed.
mise provides `make` and `cmake` but not the compiler itself, so make sure one
