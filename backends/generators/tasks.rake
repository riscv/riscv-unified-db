...
namespace :gen do
  ...
  task capstone: "#{$root}/gen/capstone" do
    ...
    cfg_arch = resolver.cfg_arch_for(config_name)
    csr_dir = cfg_arch.path / "csr"
    output_file = File.join(output_dir, "csr_switch.c")

    # Run the Capstone CSR switch generator Python script
    sh "uv run #{$root}/tools/python-packages/udb-capstone/generate_csr_switch.py " \
       "--csr-dir=#{csr_dir} --arch=BOTH --output=#{output_file}"
  end
end
