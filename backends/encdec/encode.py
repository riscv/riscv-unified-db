...
def set_bits(binary_str, positions, value):
    ...
    print(f"#    Setting bits at positions {positions} to value {value} (field width {field_w}) in binary string: {binary_str}")
    ...
    for i, pos in enumerate(positions_rev):
        ...
        if idx < 0 or idx >= instruction_width:
            print(f"#    ERROR: Bit position {pos} out of range for instruction width {instruction_width}")
            continue
