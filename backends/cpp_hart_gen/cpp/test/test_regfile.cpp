...
void allow_lower_privilege_fetch(udb::HartBase<udb::IssSocModel>* hart) {
  udb::CsrBase* pmpaddr0 = hart->csr("pmpaddr0");
  udb::CsrBase* pmpcfg0 = hart->csr("pmpcfg0");
  REQUIRE(pmpaddr0 != nullptr);
  REQUIRE(pmpcfg0 != nullptr);
  REQUIRE(pmpaddr0->sw_write(udb::Bits<64>{0x7fffffffffffff},
                             udb::Bits<8>{64}));
  REQUIRE(pmpcfg0->sw_write(udb::Bits<64>{0x1f}, udb::Bits<8>{64}));
}
...
TEST_CASE("seed enforces lower-privilege access controls", "[seed]") {
  ...
  hart->reset(0);
  allow_lower_privilege_fetch(hart);
  ...
  hart->reset(0);
  allow_lower_privilege_fetch(hart);
  udb::CsrBase* mseccfg = hart->csr("mseccfg");
  REQUIRE(mseccfg != nullptr);
  REQUIRE(mseccfg->sw_write(udb::Bits<64>{0x300}, udb::Bits<8>{64}));
  REQUIRE(mseccfg->hw_read(udb::Bits<8>{64}).to_defined().get() == 0x300);
  REQUIRE(soc.poll_count == 0);
